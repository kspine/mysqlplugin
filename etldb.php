...
class EtlDb {
    ...
    public function __construct($mysql, $mongodb) {
        ...
        try {
            ...
            $options = array();
            if (!empty($mongodb['user']) && !empty($mongodb['password'])) {
                $options['username'] = $mongodb['user'];
                $options['password'] = $mongodb['password'];
            }
            
            
            $conn = new MongoClient("mongodb://{$this->mongodb['hostname']}:{$this->mongodb['port']}", $options);
            $this->db_mongo = $conn->selectDB($this->mongodb['dbname']);//自动选择数据库
        } catch (Exception $e) {
    ...
    private function _saveToMongo($tableName, $stepPos) {
        ...
        $mongoTable = $this->db_mongo->selectCollection($tableName);
        $this->db_mongo->createCollection($tableName);

        $this->slog(sprintf("process start %d table: %s (%d rows)......", $stepPos, $tableName, $result->num_rows));
        if ($this->_options['ClearData']) {//导入之前清除mongodb以前旧的数据
            $mongoTable->remove();//清除全部记录
        ...
        while($row = $result->fetch_array(MYSQLI_ASSOC)) {
            ...
            try {
                if ($this->_options['NoBatch']) {//单条插入
                    $mongoTable->insert($row);
                } else {//批量插入
                    $write[] = $row;
                    if ($batchPos >= 10000) {//以万记录为单位批量插入
                        $mongoTable->batchInsert($write);
                        $write = array();
                        $batchPos = 0;
                    }
                }
    ...
    private function _convertDataType($fieldType, $row) {
        ...
        foreach ($row as $fieldName => $value) {
            ...
            if ($fieldType[$fieldName] == 'int') {
                $row[$fieldName] = new MongoInt64($value);
